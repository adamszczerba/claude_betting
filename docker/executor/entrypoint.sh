...
# Sets up WireGuard VPN tunnel then launches the executor poll loop.
# Follows the same pattern as docker/coincasino/entrypoint.sh.
set -e

CONF=/etc/wireguard/wg0.conf
CLEAN_CONF=/tmp/wg0-clean.conf

echo "[executor] Configuring WireGuard manually..."
...
if [ -n "$WG_DNS" ]; then
    ...
    echo "[executor] DNS: Docker (127.0.0.11) primary, VPN ($DNS1) fallback"
fi

echo "[executor] WireGuard tunnel active:"
wg show wg0

echo ""
echo "[executor] Public IP via VPN:"
curl -s --max-time 15 https://ifconfig.me || echo "(could not reach ifconfig.me)"

echo ""
echo "[executor] Starting executor in EXECUTION_MODE=${EXECUTION_MODE:-dry_run} ..."
exec python /app/execution/executor_loop.py "$@"
